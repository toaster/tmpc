...
func (s *PlayerStatus) MinSize() fyne.Size {
	return s.minSize(s)
}

func (s *PlayerStatus) Resize(size fyne.Size) {
	s.resize(s, size)
}

func (s *PlayerStatus) Update(cover image.Image, song *mpd.Song, elapsed int, playing bool) {
	if s.ticker != nil {
		s.ticker.Stop()
	}
	s.cover = cover
...
func (r *playerStatusRenderer) Refresh() {
	if r.s.song != r.song {
		r.song = r.s.song
		if r.s.cover != r.aCover.Image {
			r.aCover.Image = r.s.cover
