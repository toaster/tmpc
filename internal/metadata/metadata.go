...
func ReducedTitle(title string, language Language) string {
...
// Matcher describes a matcher for string values.
...
var numReplacements map[Language]map[string]string
var reduceRegexp *regexp.Regexp
var titleReplacers map[Language]*strings.Replacer

func init() {
	generalTitleReplacer = strings.NewReplacer("‐", "", "-", "", "  ", " ", "'", "", "’", "", "‘", "")
	numRegexp = regexp.MustCompile(`\b\d+\b`)
	numReplacements = map[Language]map[string]string{
		LanguageDE: {"0": "null", "1": "eins", "2": "zwei", "3": "drei", "4": "vier", "5": "fuenf", "6": "sechs", "7": "sieben", "8": "acht", "9": "neun", "10": "zehn", "11": "elf", "12": "zwoelf"},
		LanguageEN: {"0": "zero", "1": "one", "2": "two", "3": "three", "4": "four", "5": "five", "6": "six", "7": "seven", "8": "eight", "9": "nine", "10": "ten", "11": "eleven", "12": "twelve"},
	}
	reduceRegexp = regexp.MustCompile(`\(.*\)`)
	titleReplacers = map[Language]*strings.Replacer{
		LanguageDE: strings.NewReplacer("ä", "ae", "ö", "oe", "ü", "ue", "ß", "ss", "&", "und"),
		LanguageEN: strings.NewReplacer("&", "and"),
	}
}

func replaceNums(in string, lang Language) string {
